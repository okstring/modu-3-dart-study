...
- **MVC**: 가장 전통적인 패턴으로 구현이 간단하나 컴포넌트 간 결합도가 높음. state
- **MVP**: 테스트 용이성이 높으며 View와 Presenter는 일반적으로 1:1 관계. 비즈니스 분리
- **MVVM**: 데이터 바인딩으로 코드량 감소, ViewModel은 View에 대해 알지 못함. 양방향 바인딩. notifier
