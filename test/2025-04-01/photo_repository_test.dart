...
class MockPhotoDataSource implements PhotoDataSource {
  ...
  Future<List<Map<String, dynamic>>> fetchPhotos(String path) async {
    return mockJsonPhotos;
  }
}

void main() {
  PhotoDataSource mockPhotosDataSource = MockPhotoDataSource();
  PhotoRepository photoRepository = PhotoRepositoryImpl(
    photoDataSource: mockPhotosDataSource,
  );

  group('Photo Repository 테스트', () {
    test('getPhotos() -> 정상적으로 Photos 반환해야한다.', () async {
      const albumId = 1;
      final mockPhotos =
