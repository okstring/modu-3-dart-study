...
class MockCommentDataSource implements CommentDataSource {
  ...
  Future<List<Map<String, dynamic>>> fetchComments(String path) async {
    return mockJsonComments;
  }
}

void main() {
  CommentDataSource mockCommentsDataSource = MockCommentDataSource();
  CommentRepository commentRepository = CommentRepositoryImpl(
    commentDataSource: mockCommentsDataSource,
  );

  group('Comment Repository 테스트', () {
    test('getComments() -> 정상적으로 Comments 반환해야한다.', () async {
      const postId = 1;
      final mockComments =
          MockCommentDataSource.mockJsonComments
